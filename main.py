...
app = FastAPI(title="pdfduck API", version="5.2.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
_LABEL_FRAGMENTS = frozenset([
    'invoice', 'date', 'value', 'number', 'code', 'rate', 'currency',
    'total', 'taxable', 'exchange', 'price', 'weight', 'measure',
    'loading', 'discharge', 'destination', 'departure', 'quantity',
    'description', 'address', 'detail', 'status',
])

# All-caps strings matching these patterns are section headers, not data values
_SECTION_HEADER_RE = [
    r'^.*DETAILS$',
    r'^DECLARATION',
]
...
_COUNTRY_KEYWORDS = [
    ('UNITEDARABEMIRATES', 'UAE'), ('UNITEDARAB', 'UAE'), ('UNITEDAR', 'UAE'),
    ('DUBAI', 'UAE'), ('ABUDHABI', 'UAE'), ('SHARJAH', 'UAE'),
    ('NEWZEALAND', 'New Zealand'), ('UNITEDSTATES', 'United States'),
    ('UNITEDKINGDOM', 'United Kingdom'), ('CANADA', 'Canada'),
    ('SINGAPORE', 'Singapore'), ('GERMANY', 'Germany'), ('FRANCE', 'France'),
    ('CHINA', 'China'), ('JAPAN', 'Japan'), ('HONGKONG', 'Hong Kong'),
    ('THAILAND', 'Thailand'), ('MALAYSIA', 'Malaysia'),
    ('NETHERLANDS', 'Netherlands'), ('BELGIUM', 'Belgium'),
    ('ITALY', 'Italy'), ('SPAIN', 'Spain'), ('SOUTHAFRICA', 'South Africa'),
]
...
def is_data_value(val: str) -> bool:
    ...
    # Pure all-caps word (no digits/punctuation)
    if nospace and nospace == nospace.upper() and re.match(r'^[A-Z]+$', nospace):
        if len(nospace) > 15:
            return False
        if any(re.match(p, nospace) for p in _SECTION_HEADER_RE):
            return False
    # PascalCase field label: 2+ pascal words, no digits, contains a known label fragment
    if re.match(r'^([A-Z][a-z]+){2,}$', s) and not re.search(r'\d', s):
        if any(frag in s.lower() for frag in _LABEL_FRAGMENTS):
            return False
    return True


def detect_country(addr: str) -> Optional[str]:
    if not addr:
        return None
    norm = re.sub(r'[\s\-_/]+', '', addr).upper()

    for kw, name in _COUNTRY_KEYWORDS:
        if kw in norm:
            return name

    if re.search(r'AUSTRALIA(?!N)', addr.upper()):
        return 'Australia'
    # AU state code must be followed by 4-digit postcode to avoid substring false positives
    if any(re.search(s + r'\d{4}', norm) for s in _AU_STATES):
        return 'Australia'
    if norm.endswith('AUS'):
        return 'Australia'

    # US state code immediately before 5-digit ZIP
    for state in _US_STATES:
        if re.search(state + r'\d{5}', norm):
            return 'United States'

    # Canadian postal code: A1A1A1 pattern
    if re.search(r'[A-Z]\d[A-Z]\d[A-Z]\d', norm):
        return 'Canada'
...
class ExtractionEngine:
    ...
    def _build_kv_index(self) -> None:
        for table in self.tables:
            ...
            for r_idx, row in enumerate(table):
                ...
                # Handles the page-2 invoice section where labels and values sit
                # on separate consecutive rows.
                if r_idx + 1 < n_rows:
                ...
                # Handles 4-col [key|val|key|val] and 6-col [key|None|val|...].
                # Only sets a key if Strategy C hasn't already claimed it.
    ...
    def extract(self) -> Dict[str, Any]:
        ...
        d['consignee_country'] = detect_country(ca)
...
@app.get('/')
async def root():
    return {'service': 'pdfduck API', 'version': '5.2.0',
